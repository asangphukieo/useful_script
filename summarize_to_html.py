...
def get_input_seq(Input_seq,ID):
    return Input_seq +"/"+str(ID)+ ".fasta"
...
def summarize_GLASSgo_results(GLASSgo_folder, ID):
    GLASSgo_result = GLASSgo_folder +"/"+ "GLASSgo_output_" +ID+ ".fasta"
...
def main():
    ...
    # global parameters
    Input_seq = "SRNA_INPUT/SRNA_INPUT" 
    GLASSgo_folder = "01_GLASSgo_Results"
    GLASSgo2CopraRNA_folder = "02_GLASSgo2CopraRNA"
    CopraRNA2_simp_folder = "03_CopraRNA_Results_simplified"
    ...
    Organism_label = sys.argv[1]

    # parse fasta to get all sRNA IDs
    ###############################
    single_sRNA_fasta='./SRNA_INPUT/SPLIT_FASTA/'
    cmd = 'ls '+single_sRNA_fasta
    ps = subprocess.Popen(cmd,shell=True,stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, close_fds=True)
    list_fasta = ps.stdout.read().split('\n')
    sRNA_IDs=[]
    for sRNA in list_fasta:
        if sRNA != '':
            sRNA_IDs.append(sRNA.split('.')[0])
    #print(sRNA_IDs)
    ###############################
    ...
    with open("summarized_information.html", "w") as oh:
        ...
        for ID in sRNA_IDs:
            ...
            """.format(
                sRNA_ID = get_input_seq(Input_seq,ID),
